    ...
    <div class="agent-excellent-content">
      ...
      <div class="excellent-list">
        <el-carousel height="200px">
          <el-carousel-item
            v-for="(item, index) in excellentData"
            :key="index"
          >
            <div class="swiper">
              <div
              ...
              >
                ...
                <p class="prem"> {{$t('上月佣金')}}：¥ {{ list.month_fee }}</p>
                <p class="all"> {{$t('累计开户')}}：{{ list.members }} </p>
                <p class="times">
                  <em>{{$t('已运营')}}</em>
                  <span v-if="parseInt(list.months / 12) != 0">{{ parseInt(list.months / 12) }}{{$t('年')}}</span>
                  <span>{{ list.months % 12 }}</span>{{$t('月')}}
                </p>
              </div>
            </div>
          </el-carousel-item>
